...
其实如果大家认真做了[135. 分发糖果](https://programmercarl.com/algo/greedy/0135-candy.html)，就会发现和此题有点点的像。

在[135. 分发糖果](https://programmercarl.com/algo/greedy/0135-candy.html)我就强调过一次，遇到两个维度权衡的时候，一定要先确定一个维度，再确定另一个维度。
...
在贪心系列开篇词[关于贪心算法，你该了解这些！](https://programmercarl.com/algo/greedy/greedy-basics.html)中，我已经讲过了这个问题了。

刷题或者面试的时候，手动模拟一下感觉可以局部最优推出整体最优，而且想不到反例，那么就试一试贪心，至于严格的数学证明，就不在讨论范围内了。

如果没有读过[关于贪心算法，你该了解这些！](https://programmercarl.com/algo/greedy/greedy-basics.html)的同学建议读一下，相信对贪心就有初步的了解了。
...
关于本题使用数组还是使用链表的性能差异，我在[贪心算法：根据身高重建队列（续集）](https://programmercarl.com/algo/greedy/reconstruct-queue-vector-internals.html)中详细讲解了一波

## 总结

关于出现两个维度一起考虑的情况，我们已经做过两道题目了，另一道就是[135. 分发糖果](https://programmercarl.com/algo/greedy/0135-candy.html)。

**其技巧都是确定一边然后贪心另一边，两边一起考虑，就会顾此失彼**。

这道题目可以说比[135. 分发糖果](https://programmercarl.com/algo/greedy/0135-candy.html)难不少，其贪心的策略也是比较巧妙。
...
<Tabs items={["Java","Python","Go","JavaScript","Rust","C","TypeScript","Scala","C#"]}>
<Tab>
