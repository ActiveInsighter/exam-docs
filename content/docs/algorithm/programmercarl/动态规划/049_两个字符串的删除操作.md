...
本题和[动态规划：115.不同的子序列](https://programmercarl.com/algo/dynamic-programming/0115-distinct-subsequences.html)相比，其实就是两个字符串都可以删除了，情况虽说复杂一些，但整体思路是不变的。
...
本题和[动态规划：1143.最长公共子序列](https://programmercarl.com/algo/dynamic-programming/1143-longest-common-subsequence.html)基本相同，只要求出两个字符串的最长公共子序列长度即可，那么除了最长公共子序列之外的字符都是必须删除的，最后用两个字符串的总长度减去两个最长公共子序列的长度就是删除的最少步数。
