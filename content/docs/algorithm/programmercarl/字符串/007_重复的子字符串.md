...
如果我们做过[28.实现strStr](https://programmercarl.com/algo/string/0028-find-the-index-of-the-first-occurrence-in-a-string.html)题目的话，其实就知道，**实现一个 高效的算法来判断 一个字符串中是否出现另一个字符串是很复杂的**，这里就涉及到了KMP算法。
...
- [文字讲解版：KMP算法](https://programmercarl.com/algo/string/0028-find-the-index-of-the-first-occurrence-in-a-string.html)
...
可以看这里：[KMP讲解](https://programmercarl.com/algo/string/0028-find-the-index-of-the-first-occurrence-in-a-string.html)，再去回顾一下。
...
next 数组记录的就是最长相同前后缀（[字符串：KMP算法精讲](https://programmercarl.com/algo/string/0028-find-the-index-of-the-first-occurrence-in-a-string.html)）， 如果`next[len - 1] != -1`，则说明字符串有最长相同的前后缀（就是字符串里的前缀子串和后缀子串相同的最长长度）。

最长相等前后缀的长度为：`next[len - 1] + 1`。(这里的next数组是以统一减一的方式计算的，因此需要+1，两种计算next数组的具体区别看这里：[字符串：KMP算法精讲](https://programmercarl.com/algo/string/0028-find-the-index-of-the-first-occurrence-in-a-string.html))
...
![459.重复的子字符串_1](https://file1.kamacoder.com/i/algo/459.%E9%87%8D%E5%A4%8D%E7%9A%84%E5%AD%90%E5%AD%97%E7%AC%A6%E4%B8%B2_1.png)
...
<Tabs items={["Java","Python","Go","JavaScript","TypeScript","Swift","Rust","C#","C"]}>
<Tab>
