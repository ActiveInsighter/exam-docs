...
本题和[113.路径总和II](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)是类似的思路，做完这道题，可以顺便把[113.路径总和II](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)和[112.路径总和](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)做了。

结合112.路径总和 和 113.路径总和II，我在讲了[二叉树：递归函数究竟什么时候需要返回值，什么时候不要返回值？](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)，如果大家对二叉树递归函数什么时候需要返回值很迷茫，可以看一下。

接下来在看本题，就简单多了，本题其实需要使用回溯，但一些同学可能都不知道自己用了回溯，在[二叉树：以为使用了递归，其实还隐藏着回溯](https://programmercarl.com/algo/other/recursion-and-backtracking-in-binary-trees.html)中，我详细讲解了二叉树的递归中，如何使用了回溯。
...
如果对递归三部曲不了解的话，可以看这里：[二叉树：前中后递归详解](https://programmercarl.com/algo/binary-tree/recursive-binary-tree-traversal.html)

- 确定递归函数返回值及其参数

这里我们要遍历整个二叉树，且需要要返回值做逻辑处理，所有返回值为void，在[二叉树：递归函数究竟什么时候需要返回值，什么时候不要返回值？](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)中，详细讲解了返回值问题。

参数只需要把根节点传入，此时还需要定义两个全局遍历，一个是result，记录最终结果，一个是vectorpath。
...
![](https://file1.kamacoder.com/i/algo/129.求根到叶子节点数字之和.png)
...
<Tabs items={["Java","Python","Go","JavaScript","TypeScript","C"]}>
<Tab>
