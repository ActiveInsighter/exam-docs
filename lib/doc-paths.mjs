...
export function getShortDocSlugs(relativePath) {
  ...
  const slugs = sourceSegments.map((segment, index) => {
    if (index === 0 || (SAFE_SOURCE_SEGMENT.test(segment) && segment.length <= 40)) {
      // Keep the logical slug decoded. Next.js/Fumadocs encode non-ASCII
      // route segments when they become URLs; encoding here makes the route
      // parameter contain literal percent escapes and breaks lookup.
      return segment;
    }
